...
export default {
    ...
    actions: {
        ...
        async get_product({ commit },id ) {
            let response = await axios.get(`/product/${id}`,{headers:{
                Authorization: "Bearer " + localStorage.getItem("token")
                }});
        ...
        async get_product_paginated({ commit }, page ) {
            console.log(page)
            let response = await axios.get('/productpaginated?page='+page,{headers:{
                Authorization: "Bearer " + localStorage.getItem("token")
                }});
            commit("GET_PRODUCTS_PAGINATED", response.data.data);
        },
        async get_product_subcategorie({ commit },id ) {
            let response = await axios.get(`/product/subcategorie/${id}`,{headers:{
                Authorization: "Bearer " + localStorage.getItem("token")
                }});
            commit("GET_PRODUCT", response.data.data);
        },
        ...
        async update_products({ commit },data){
            let response = await axios.post(`/product/${data.id}`,data,{headers:{
